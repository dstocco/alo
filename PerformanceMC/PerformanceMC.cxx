                                  ...
                                  float chi2Cut)
{
  ...
  for (auto& hit : trackHits) {
    ...
    if (nMatched > 0) {
      ...
      std::cerr << "PerformanceMC::checkClusters:\n" << hit << " => Clusters:\n";
      for (auto& cluster : recoClusters) {
...
//______________________________________________________________________________
bool PerformanceMC::checkParamFromHits(double pTot,
                                       const std::vector<Hit>& trackHits)
{
  /// Fills the histograms with the slope and impact parameters
  /// obtained froms the hits on the first and last chamber.
