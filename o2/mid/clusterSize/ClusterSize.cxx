...
std::vector<PreCluster> ClusterSize::selectPreClusters(const std::vector<PreCluster>& preClusters) const
{
  ...
  std::vector<PreCluster> pcNBP(preClusters.size()), pcBP(preClusters.size()), selected(preClusters.size());
  for (int ide = 0; ide < detparams::NDetectionElements; ++ide) {
    pcNBP.clear();
    pcBP.clear();
    ...
    if (pcNBP.size() <= 1 && areContiguous(pcBP)) {
      std::copy(pcBP.begin(), pcBP.end(), std::back_inserter(selected));
      std::copy(pcNBP.begin(), pcNBP.end(), std::back_inserter(selected));
    }
  }
  return selected;
}
