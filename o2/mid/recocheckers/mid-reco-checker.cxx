...
std::vector<o2::mid::ColumnData> getColumnDataNonMC(const o2::mid::DigitsMerger& dm)
{
  std::vector<o2::mid::ColumnData> vec;
  auto ref = dm.getColumnData();
  vec.insert(vec.begin(), ref.begin(), ref.end());
  return vec;
}
...
int main(int argc, char* argv[])
{
  ...
  for (auto input : inputfiles) {
    ...
    while (reader.read(trackRefsVector, nTracksPerEvent)) {
      ...
      preClusterizer.process(getColumnDataNonMC(digitsMerger));
