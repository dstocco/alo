...
int main(int argc, char* argv[])
{
  ...
  std::string orbitCorrFilename = "orbit-correspondence.txt";
  ...
  generic.add_options()
          ...
          ("orbit-correspondence-filename",po::value<std::string>(&orbitCorrFilename),"orbit correspondence")
  ...
  std::ifstream orbitCorrFile(orbitCorrFilename.c_str());
  if (orbitCorrFile.is_open()) {
    std::string line;
    while (std::getline(orbitCorrFile, line)) {
      auto idx = line.find(" ");
      uint32_t orb1 = std::stoul(line.substr(0, idx).c_str());
      uint64_t orb2 = std::stoul(line.substr(idx + 1).c_str());
      orbitCorrespondence[orb1] = orb2;
    }
  }
  ...
  for (auto ievt = 0; ievt < tree->GetEntries(); ++ievt) {
    ...
    for (auto& rof : *rofRecords) {
      ir = rof.interactionRecord;
      auto orbitFound = orbitCorrespondence.find(ir.orbit);
      uint64_t ts = ir.toLong();
      if (orbitFound != orbitCorrespondence.end()) {
        ts = orbitFound->second + ir.bc;
      }
