...
/// \file   FireProbabilityAnalysis.cxx
/// \brief  Implementation of the task to assess the strip fire probability for MID
/// \author Diego Stocco <Diego.Stocco at cern.ch>
/// \date   13 August 2019

#include "FireProbabilityAnalysis.h"
...
FireProbabilityAnalysis::FireProbabilityAnalysis(const char* filename)
  : mFireProbability()
{
  /// Default constructor
  readFireProbability(filename);
}

//______________________________________________________________________________
bool FireProbabilityAnalysis::readFireProbability(const char* filename)
{
  ...
  for (int ide = 0; ide < detparams::NDetectionElements; ++ide) {
    ...
    for (int icath = 0; icath < 2; ++icath) {
      ...
      int ipt = 0;
      double err;
      mFireProbability.push_back({});
      std::stringstream grName;
      grName << "fired_probability_deId_" << ide << "_cath_" << icath;
      mFireProbability.back().SetName(grName.str().c_str());
      ...
      for (int ibin = 1; ibin <= nbins; ++ibin) {
        if ( histo->GetBinContent(ibin) > 0. ) {
          ...
          double xmax = histo->GetXaxis()->GetBinUpEdge(ibin);
          // mFireProbability.back().SetPoint(ipt, 0.5 * (xmax + xmin), content);
          // mFireProbability.back().SetPointError(ipt, 0.5 * (xmax - xmin), err);
          mFireProbability.back().SetPoint(ipt, xmin, content);
          mFireProbability.back().SetPointError(ipt, 0., err);
          xmin = xmax;
          ++ipt;
        }
      }
...
//______________________________________________________________________________
bool FireProbabilityAnalysis::process(const ChamberResponse& response)
{
  ...
  std::array<double, 2> maxFit{34., 120.};
  ...
  for (int ide = 0; ide < detparams::NDetectionElements; ++ide) {
    for (int icath = 0; icath < 2; ++icath) {
      ...
      int idx = 2 * ide + icath;
      mFireProbability[idx].Fit(&func, "QN0", "", 0., maxFit[icath]);
      mFireProbability[idx].GetListOfFunctions()->Add(func.Clone());
...
//______________________________________________________________________________
bool FireProbabilityAnalysis::saveResults(const char* filename) const
{
