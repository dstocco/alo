...
int main(int argc, char* argv[])
{
  ...
  generic.add_options()
          ...
          ("output", po::value<std::string>(&outFilename)->default_value("MIDFireProbabilityFit.root"),"basename of output file with tuned parameters");

  po::options_description hidden("hidden options");
  hidden.add_options()
          ("input", po::value<std::string>(),"FireProbability output");
  // clang-format on
  ...
  o2::mid::FireProbabilityAnalysis fireProbAnalysis(inputfile.data());
  fireProbAnalysis.process(resp);
  fireProbAnalysis.saveResults(outFilename.data());
